define(["jQuery", "kendo", "config", "app/utils"], function ($, kendo, config, utils) {
    ...
    var DataSourceConfig = function (url, sortField, options) {
        ...
        console.log("DataSourceConfig", url);
        $.extend(this, options || {});
    };
    ...
    return {
        ...
        homeList: new kendo.data.DataSource({
            transport: {
                read: {
                    url: config.homeUrl,
                    dataType: "json"
        ...
        branchList: new kendo.data.DataSource({
            transport: {
                read: {
